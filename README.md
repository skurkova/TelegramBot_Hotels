...
В связи с ограничениями доступа к RapidAPI из некоторых регионов, 
проект поддерживает режим демонстрации с реальными данными API.

**Переключение режимов** - в файле `.env` укажите:

`MOCK_API=True` - демонстрация без API 

`MOCK_API=False` - с реальным RapidAPI

В mock-режиме бот использует сохранённые ответы API из файлов JSON:
- `mock_data/possible_cities.json` — поиск городов
- `mock_data/possible_hotels.json` — список отелей  
- `mock_data/hotel_options.json` — детали отеля

Это позволяет демонстрировать полный функционал бота без доступа к внешнему API.